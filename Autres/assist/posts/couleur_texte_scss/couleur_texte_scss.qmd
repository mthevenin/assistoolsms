...
```
Et on va appliquer ce style à du texte:

-   Soit comme précédemment avec une balise `<span>`

``` md
Je veux que <span style=".text-red"> TEXTE</span> soit en rouge 
```

***Résultat***: Je veux que [TEXTE]{style=".text-red"} soit en rouge

-   Soit, **ce qui est fortement recommandé avec Quarto**, avec un attribut dont la syntaxe est `[TEXTE]{style}`
...
::: callout-tip
### Ajout d'attributs et de classes
...
* Ces définitions préalables sont appelées **variables SASS**^[Oui je sais si on a travaillé avec Sas cela peut porter à confusion, d'autant que la syntaxe de ces variables ressemblent à s'en méprendre aux macro variables de cette application], que l'on peut soit générer facilement: `$couleur1=red;` ou utiliser les couleurs du thème choisi. 
  ...
  * On peut peut récupérer facilement ces couleurs sur le site [bootswatch](https://bootswatch.com/) comme indiquer   * On peut récupérer le squelette d'un fichier `.scss` pour les 26 à cette adresse [[fichiers scss des 26 thèmes  pour les couleurs](https://github.com/mthevenin/quarto/tree/main/scss_color_theme)]
