...
abstract: | 
 Un graphique devient très facilement illisible lorsque trop d'éléments sont présents, par exemple pour des courbes, au-delà de 5-6. Ce problème est appelé **effet spaghetti** . Pour contrôler cette difficulté on préconise un graphique de type **small-multiples** (graphique combiné). On peut améliorer cette visualisation avec une technique de highlighting: chaque sous graphique est mis en avant en terme de couleur et d'épaisseur  tout en conservant en arrière plan la visualisation des autres courbes avec une épaisseur plus petite et une couleur unique (par exemple en gris).
...
* Des options seront ajoutées ultérieurement (options `thème()`, choix des groupes ....).
...
spag_plot(df,                     # Base de données au format long
          ...
          col_line_bg = "#919aa1",   # Couleur lignes secondaires
